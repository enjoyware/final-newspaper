var start = (function (exports) {
    ...
    var renderArticle = function renderArticle(data) {
      ...
      var dateView = new hooks(data.published).format('MM/DD/YYYY hh:mm A');
      return "\n        <article class=\"col-md-".concat(col, "\">\n            <div class=\"list-article\">\n                <h2>").concat(data.title, "</h2>\n                <small>Published on: ").concat(dateView, "</small>\n                <div>\n                    ").concat(data.content.html, "\n                </div>\n            </div>\n            <a href=\"?article=").concat(data.id, "\">Read More</a>\n        </article>\n    ");
    };

    var loadArticles = function loadArticles(query) {
      $.post({
        url: 'https://api-uswest.graphcms.com/v1/cjvx3xfw0b7q401ghnpl5g0zs/master',
        data: JSON.stringify({
          "query": query
        }),
    ...
    */
    ...
    var genereateCreateArticleQL = function genereateCreateArticleQL(data) {
      return "\n    mutation {\n        createArticle(\n            data: {\n                status: PUBLISHED\n                title: \"".concat(data.title, "\"\n                category: ").concat(data.category, "\n                content: \"").concat(data.content, "\"\n                published: \"").concat(data.published, "\"\n            }\n        ) {\n            id\n            title\n        }\n    }\n");
    };

    var setToken = function setToken(user, pass) {
      sessionStorage.setItem('token', btoa("".concat(user, ":").concat(md5(pass))));
    };

    var login = function login() {
      $.ajaxSetup({
        beforeSend: function beforeSend(xhr) {
          xhr.setRequestHeader("Authorization", "Basic ".concat(sessionStorage.getItem('token')));
        }
      });
    ...
    var createArticle = function createArticle() {
      ...
      $('form button').on('click', function (event) {
        ...
        var createArticleQl = genereateCreateArticleQL(data);
        $.post('/api/form', JSON.stringify({
          "query": createArticleQl
        }), function (res) {
          if (res.status === 'SUCCESS') {
